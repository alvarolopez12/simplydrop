...
# -------------------------monthly_summary.py---------------------------------
# Script que se ejecuta  dia 1 de cada mes a las 9 am: 0 9 1 * *
...
def main():
    ...
    first_of_two_month_ago = (first_of_last_month -
                              timedelta(days=1)).replace(day=1)
    ...
    for user in users:
        ...
        items = Items.query.filter(Items.shop_id == shop.id,
                                   Items.added_on > first_of_last_month,
                                   Items.added_on < first_of_this_month).all()


        orders_last_period = Orders.query.filter(Orders.shop_id == shop.id,
                                                 Orders.added_on >= first_of_two_month_ago,
                                                 Orders.added_on < first_of_last_month).all()
        ...
        send_email.send_order_email_summary(user.name, user.email, shop.url,
